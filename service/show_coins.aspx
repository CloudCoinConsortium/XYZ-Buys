...
<script language="c#" runat="server">
    ...
    public void Page_Load(object sender, EventArgs e)
    {
		...
		if (System.IO.File.Exists( Server.MapPath("..") + @"\accounts\" + PasswordFolder + @"\" + account + @".txt")){
			   ...
			   string pkReadFromFile = System.IO.File.ReadAllText( Server.MapPath("..") + @"\accounts\" + PasswordFolder + @"\" + account + @".txt");
			   serviceResponse.account = account;
		
			if ( pkReadFromFile != pk )
			{
            serviceResponse.status = "fail";
	...
	//End of checking passwords
        ...
        string pathToAccount = HttpRuntime.AppDomainAppPath.ToString() + @"\accounts\" + pk + @"\";
        FileUtils fileUtils = new FileUtils(pathToAccount);

///serviceResponse.status = pathToAccount;
